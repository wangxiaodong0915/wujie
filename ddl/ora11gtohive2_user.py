...
def transform_hive(tablename, table_comment, filepath):
    """
...
(   """ % table_name.upper()
        ...
        for i in hive_comment:
            if i[1]:
                if i[2] in ("DATA","TIMESTAMP","STRING"):
...
"""
        ...
        #开始写文件
        file = open(os.path.join(filepath,table_name+'.ddl'), 'w')
        try:
            file.write(sql.lower())
            print("表结构已经写入 %s" % table_name+'.ddl')
        finally:
            file.close()
        result = []
        for i in hive_comment:
...
for i in table_list:
    try:
        ora_table_com = get_comment(cursor, i)
        wttoExcel(f_oracle, i, ora_table_com)
        hive_table_com = transform_hive(i, ora_table_com, filepath)
        wttoExcel(f_hive, i, hive_table_com)
        print('index : %d' % count )
    except Exception:
        print(Exception)
        err_list.append(i)
